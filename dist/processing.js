...
exports.processDisconnection = processDisconnection;
const MAX_TEMPERATURE = 45;
const MIN_TEMPERATURE = 25;
const boilers = new Map();
const apps = new Map();
 ...
 */
function processBoilerInit(message, ws) {
 ...
 * @param ws The WebSocket connection to the boiler
 */
function processBoilerUpdate(message, ws) {
    const boilerId = message.boilerId;
    const temperature = message.temperature;
    const currentTemperature = message.currentTemperature;
    const isOn = message.isOn;
 ...
 */
function processAppInit(message, ws) {
    const appId = message.appId;
...
// MARK: Disconnections
function processDisconnection(ws) {
 ...
 */
function processBoilerDisconnection(boilerId) {
    const boiler = boilers.get(boilerId);
    if (!boiler) {
 ...
 */
function processAppDisconnection(appId) {
